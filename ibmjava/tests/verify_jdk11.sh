...
do
	...
	v)
		...
		major_ver="$OPTARG"
		((major_ver == 11 || major_ver == 9)) || usage
...
function get_full_version_from_meta_info() {
	...
	yml_file="public.dhe.ibm.com/meta/11.0/jdk/linux/x86_64/index.yml"
...
function match_shasums() {
	...
	do
		for arch in $arches
		do
			yml_file="public.dhe.ibm.com/meta/11.0/$pkg/linux/$arch/index.yml"
			pack_url=`grep -A 1 "$full_version" $yml_file | grep "uri:" | awk '{ print $2 }'`
			rm -f ibm-java.bin
			echo -n "Downloading $pkg for $arch from $pack_url..."
			...
			echo
			if [ "$arch" == "x86_64" ]; then
				./java-test/*/bin/java -version 2>&1 | tee version-info/$version-$pkg.txt
			fi
			echo
		done
	done
...
rootdir="$PWD/../"
update_file=$rootdir/update_jdk11.sh
